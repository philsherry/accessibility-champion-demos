...
export default defineConfig({
  ...
  // fullyParallel plus the default worker count (~half the machine's cores)
  // lets all three projects' tests hit the single dev http-server at once,
  // oversubscribing it — the confirmed cause of intermittent contention
  // failures under full `npm test` runs (see TODO.md). Capping workers
  // explicitly, rather than relying on the CPU-derived default, keeps
  // concurrency within what the single-threaded dev server can reliably
  // service.
